...
(defconst flycheck-java-jde-project-file-name "prj.el")
...
(defun flycheck-java-read-jde-project (dir)
  (list :project-dir dir
        :source jde-compile-option-source
        :target jde-compile-option-target
        :paths (mapcar* #'cons
                       (jde-normalize-paths-for-lisp jde-sourcepath)
                       (list (jde-normalize-path-for-lisp jde-compile-option-directory)))
        :lib-paths (jde-normalize-paths-for-lisp (cdr jde-global-classpath))))
...
(defun flycheck-java-find-project (dir)
  ...
  (let ((eclipse-dir (locate-dominating-file dir flycheck-java-eclipse-project-file-name))
        ...
        (jde-dir (locate-dominating-file dir flycheck-java-jde-project-file-name)))
    (cond
     ((stringp jde-dir) (cons :jde jde-dir))
...
(defun flycheck-java-get-project-def (dir)
  ...
  (let* ((project (flycheck-java-find-project dir))
    ...
    (pcase (car project)
      ...
      (:jde (flycheck-java-read-jde-project root-dir))
      (:custom
...
(defun flycheck-java-make-cmd-options (p)
  ...
  (let* ((root (expand-file-name (plist-get p :project-dir)))
         ...
         (src-paths (mapcar #'cygwin-convert-file-name-to-windows (mapcar #'car paths)))
         ;(dst-paths (--map (concat root "/" (cdr it)) paths))
         (dst-paths (mapcar #'cygwin-convert-file-name-to-windows (mapcar #'cdr paths)))
         ...
         (class-path (--reduce (concat acc ";" it)
                               (-flatten (list dst-paths (mapcar #'cygwin-convert-file-name-to-windows expanded-libs))))))
    (append (list "-source"
                  (plist-get p :source)
                  "-target"
                  (plist-get p :source)
...
(flycheck-define-checker java
  ...
  :command ("java" 
            (option "-jar" flycheck-java-ecj-jar-path list cygwin-convert-file-name-to-windows)
            "-d" "none" "-Xemacs" 
